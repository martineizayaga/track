  ...
  <v-app>
    <v-content>
      <v-toolbar color="#619b5b" dark>
        ...
        <v-toolbar-title>Add New Record</v-toolbar-title>
        <v-spacer></v-spacer>
      </v-toolbar>
      <v-container>
          ...
          <v-form ref='form'>
            ...
            <v-flex xs12 sm6 md4>
              <v-menu :close-on-content-click="false" :nudge-right="40" lazy transition="scale-transition" offset-y full-width min-width="290px">
                ...
                <v-date-picker v-model="date" min-width="100px"></v-date-picker>
              </v-menu>
            </v-flex>
            <!-- end of DATE field -->
...
export default {
  ...
  methods: {
    submit () {
      ...
      // var newId = this.recordsList.length + 1;
      ...
      this.recordWidgets.forEach(function (widget, index) {
        ...
        // console.log(vm.$refs.w[index].getValue())
      })
